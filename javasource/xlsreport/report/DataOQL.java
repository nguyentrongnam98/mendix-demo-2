...
public class DataOQL
{
	...
	private String createOQLQuery(MxSheet mxSheet, IMendixObject inputObject, List<ColumnPreset> mxColumnLists) throws CoreException
	{
		...
		List<IMendixObject> constrainList = Core.createXPathQuery("//"
						+ MxConstraint.getType()
						+ "["
						+ MxConstraint.MemberNames.MxConstraint_MxSheet.toString()
						+ "='" + mxSheet.getMendixObject().getId().toLong() + "']")
				.setAmount(Integer.MAX_VALUE)
				.setOffset(0)
				.addSort(MxConstraint.MemberNames.Sequence.toString(), true)
				.execute(context);
			...
			switch (path.getRetrieveType())
			{
				case Reference:
					...
					if (fromObjectData.containReference(referenceName))
					{
						...
						log.trace("Searching further on existing reference: "
								+ referenceName + " with object: "
								+ toObjectName.getCompleteName());
						if (!toObjectData.getClass().isInstance(fromObjectData))
						{
							log.warn("The TO object: "
									+ toObjectName.getCompleteName()
									+ " has an reference: "
									+ referenceName
									+ " but doesn't match in subclass of object name: "
									+ fromObjectData.getCompleteName());
						}
					} else
					{
	...
	{
		List<IMendixObject> orders = Core.createXPathQuery("//"
						+ MxSorting.getType() + "["
	 ...
	 * @throws CoreException
	 */
	private HashMap<String, String> getReferenceHandling(MxSheet sheet)
	{
		List<IMendixObject> referenceList = Core.createXPathQuery("//"
